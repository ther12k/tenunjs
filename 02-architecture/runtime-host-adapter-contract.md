...
- **Unhandled promise-rejection visibility (review 13)**: each VM installs a host promise-rejection tracker. A rejection reported with no handler is recorded (bounded count, report order, reason captured as bounded owned text); attaching a handler later removes the record (handled transition). At the end of each `pump` drain, outstanding unhandled rejections fail the turn — `pump` returns `-1` with a `TJERR:EVAL` diagnostic aggregating the tracked reasons — instead of reporting a successful drain. Rejections handled within the same drain never fail the turn. This is asynchronous error reporting for the microtask system (TN-011's uncaught-error requirement), distinct from promise-to-native-future bridging, which stays out of scope. Regressions: unhandled → pump fails; catch attached in the same drain → pump succeeds; handler attached after the rejection → pump succeeds; `Promise.reject(42)` → numeric reason preserved; three simultaneous rejections → single aggregated diagnostic in report order, cleared by the next successful call.
